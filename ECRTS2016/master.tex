...
real-time task $\tau_i$ is characterized by its \emph{minimum
  inter-arrival time} (also called period) $T_i$ and its
...
model, the next job is released at time $t+T_i$.
...
not.  For over half a centry, researchers in real-time systems have
...
specified deadlines. In most of these studies, \emph{task suspensions
  are usually not allowed}. That is, after a job is released, the job
...
synchronization. However, if the suspending patterns can be
characterized to form segmented patterns, the segmented
...
The recent report by Chen et al. and the erratum by Bletsas et
...
\item We provide a general analysis framework in
  ...
  dominates the existing results in
  \cite{BletsasReport2015,huangpass:dac2015} and \cite[Pages
  164-165]{Liu:2000:RS:518501}. The key observation in the analysis
  framework is that the \emph{interference from higher-priority
    ...
    block terms}. Therefore, these existing analyses are just special
  cases in the analysis framework.  Moreover, the proof of
...
\item We develop a few strategies to decide which higher-priority
  ...
  higher-priority tasks should be classified to associate with block
...
However, there is no proof in
\cite{Liu:2000:RS:518501,DBLP:conf/rtss/RajkumarSL88} to support the
correctness of the above tests. 
...
This is the most pessimistic analysis. It is not difficult to see that
the test in Eq.~\eqref{eq:TDA-suspension} dominates that in
Eq.~\eqref{eq:TDA-SO}.

\begin{Lemma}
  The schedulability test of task $\tau_k$ in
  Eq.~\eqref{eq:TDA-suspension} dominates that in
  Eq.~\eqref{eq:TDA-SO}.
\end{Lemma}
\begin{proof}
...
\begin{theorem}
  ...
  schedulability tests in Eq.~(\ref{eq:TDA-jitter}),
  Eq.~(\ref{eq:TDA-suspension}), and Eq.~\eqref{eq:TDA-SO}.
\end{theorem}
\begin{proof}
...
Eqs.~(\ref{eq:exec_plus_idle-3})  leads to 
\begin{equation}
