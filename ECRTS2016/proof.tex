...
\end{itemize}
Note that if $t_j \not= t_{j+1}$, then $t_j = r_j$ and the job $J_j$ did not complete its execution yet. Therefore, the amount of time during which the processor may remain idle within $[t_j, t_{j+1})$ is at most $S_j$ time units.
...
{\bf Step 3: Creating a Safe Response-Time Analysis}
