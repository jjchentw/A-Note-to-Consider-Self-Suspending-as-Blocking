...
\noindent{\bf Case 0.} If all the jobs of task $\tau_j$ are released at or after $t_{j+1}$, then we define $\Psi^j$ as being identical to $\Psi^{j+1}$ and set $t_j \equals t_{j+1}$. 
...
\noindent{\bf Case 1.} If $\tau_j \in {\bf T}_1$ and $J_{j}$ did not complete its execution by $t_{j+1}$ in the schedule $\Psi^{j+1}$ (i.e., $c_j^*>0$), then $t_j \equals r_j$ and $\tau_j$ releases its jobs at the exact same instants than in $\Psi^{j+1}$ \emph{at and after} $r_j$ (i.e., it includes $J_j$ in $\Psi^j$). 

\noindent{\bf Case 2.} If $\tau_j \in {\bf T}_1$ and $J_{j}$ completes its execution before or at $t_{j+1}$ in the schedule $\Psi^{j+1}$ (i.e., $c_j^* = 0$), then $t_j \equals t_{j+1}$ and $\tau_j$ releases its jobs at the exact same instants than in $\Psi^{j+1}$ \emph{after} $r_j$ (i.e., it excludes $J_j$ in $\Psi^j$).

\noindent{\bf Case 3.} If $\tau_j \in {\bf T}_0$, then $t_j \equals t_{j+1}$ and (i) $\tau_j$ releases its jobs at the exact same instants than in $\Psi^{j+1}$ \emph{after} $r_j$ (i.e., exclude $J_j$ in $\Psi^j$), and (ii) an additional job $J_a$ with execution time $C_a \equals c_j^*$ and the same priority than $\tau_j$ is released at time $t_{j+1}$.
...
\end{Lemma}
\begin{proof}
If task $\tau_j \in {\bf T}_1$, then Case 0, 1 or 2 was applied when building $\Psi^1$ in Step 1. % and the job $J_j$ has not finished yet at
  %time $t_{j+1}$. 
...
\end{Lemma}
\begin{proof}
If task $\tau_j \in {\bf T}_0$, then Case 0 or 3 was applied when building $\Psi^1$ in Step 1. %This is the case when
  %$\tau_j \in {\bf T}_0$. 
...
%\item If the residual workload $c_j^*$ of task $\tau_j$ is larger than $0$,
  ...
  worst-case response time, the deadline of the job $J_j$ of $\tau_j$ that was not completed yet at $t_j$, must be at least $t_j + c_j^*$. %otherwise that job would violate its worst-case execution time (that has been confirmed earlier). 
...
\begin{appProof}{of Theorem \ref{theorem:general-framework}} 
