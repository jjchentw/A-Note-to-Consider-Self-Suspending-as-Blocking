...
\item $\tau_j \in {\bf T}_0$. This
  ...
  before $t_j$ with the residual workload $0 \leq c_j^* \leq C_j$ at time $t_j$.  Let $\widehat{W}_j^0(\Delta, c_j^*)$ be the upper bound on the executed workload that is contributed by $\tau_j$ from $t_j$ to $t_j+\Delta$ for any $\Delta
  \geq 0$, under a given $c_j^*$. If $c_j^*$ is $0$, it is clear that $\widehat{W}_j^0(\Delta, c_j^*)$ is $W_j^1(\Delta)$. We now consider another case when $c_j^* > 0$.
...
& \leq \ceiling{\frac{t-t_k^*+\sum_{\ell=j}^{k-1} x_\ell\sigma_\ell +(R_j-C_j)}{T_j}}C_j  
\end{align*}}for task $\tau_j$ in ${\bf T}_0$ with $x_j = 0$. Therefore, we can conclude that 
$\forall t \mid t_k^* \leq t < f_k$
\begin{equation}
...
The above inequality implies that the minimum $\theta$ with $\theta > 0$ such that
